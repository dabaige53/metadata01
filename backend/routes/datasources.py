...
def get_datasources():
    ...
    if sort == "name":
        query = query.order_by(
            Datasource.name.desc() if order == "desc" else Datasource.name.asc()
        )
    elif sort == "project_name":
        query = query.order_by(
            Datasource.project_name.desc()
            if order == "desc"
            else Datasource.project_name.asc()
        )
    elif sort == "table_count":
        query = query.order_by(
            Datasource.table_count.desc()
            if order == "desc"
            else Datasource.table_count.asc()
        )
    elif sort == "workbook_count":
        query = query.order_by(
            Datasource.workbook_count.desc()
            if order == "desc"
            else Datasource.workbook_count.asc()
        )
    elif sort == "last_refresh":
        query = query.order_by(
            Datasource.extract_last_refresh_time.desc()
            if order == "desc"
            else Datasource.extract_last_refresh_time.asc()
        )
    else:
        # 默认排序
    ...
    if ds_ids:
        ...
        table_map = {
            row[0]: {"embedded": row[1], "regular": row[2]} for row in tbl_stats
        }
    ...
    for ds in datasources:
        ...
        data["embedded_table_count"] = (
            tbl_info["embedded"] if isinstance(tbl_info, dict) else 0
        )
        data["regular_table_count"] = (
            tbl_info["regular"] if isinstance(tbl_info, dict) else 0
        )
...
def get_datasource_detail(ds_id):
    ...
    for f in source_fields:
        ...
        summary = {
            ...
            "upstream_column_name": f.upstream_column.name
            if f.upstream_column
            else None,
            # 来源信息（用于聚合后展示）
    ...
    data["tableau_url"] = build_tableau_url(
        "datasource", uri=ds.uri, luid=ds.luid, vizportal_url_id=ds.vizportal_url_id
    )
