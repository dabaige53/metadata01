...
def get_fields_no_description():
    ...
    # 转换为数组并按字段数降序排列
    groups = sorted(
        groups_map.values(), key=lambda grp: len(grp["fields"]), reverse=True
    )
    for grp in groups:
...
def get_fields_catalog():
    ...
    if dedup_method_filter:
        ...
        for value in dedup_values:
            if value == "physical_table":
                dedup_clauses.append(
                    "(urf.table_id IS NOT NULL AND (t.is_embedded = 0 OR t.is_embedded IS NULL))"
                )
    ...
    if search:
        ...
        facet_where = "WHERE (urf.name LIKE :search OR urf.description LIKE :search)"
...
def get_fields():
    ...
    facets["hasDescription"] = {
        "true": stats.has_desc or 0,
        "false": total - (stats.has_desc or 0),
    }
...
def get_field_detail(field_id):
    ...
    if unique_row:
        ...
        data = {
            ...
            "workbookName": None,
            # 血缘标签
            "lineageSource": unique_row.lineage_source
            if hasattr(unique_row, "lineage_source")
            else None,
            "penetrationStatus": unique_row.penetration_status
            if hasattr(unique_row, "penetration_status")
            else None,
        }
        ...
        data = {
            ...
            "hasDescription": bool(
                field_row.description and field_row.description.strip()
            ),
        ...
        data["all_datasources"] = [
            {
                ...
                "project_name": r[2],
                "owner": r[3],
                "is_certified": bool(r[4]) if r[4] is not None else False,
            }
            for r in ds_lineage
        ]

        # 3. 工作簿血缘
        wb_lineage = session.execute(
            text("""
