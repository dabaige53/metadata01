        ...
        groups_map[ds_name]["metrics"].append(
            {
                "id": row.id,
                "name": row.name,
                "formula": row.formula[:100] + "..."
                if row.formula and len(row.formula) > 100
                else row.formula,
            }
        )

    groups = sorted(
        groups_map.values(), key=lambda grp: len(grp["metrics"]), reverse=True
    )
    for grp in groups:
        grp["metric_count"] = len(grp["metrics"])

    return jsonify(
        {"total_count": len(rows), "datasource_count": len(groups), "groups": groups}
    )
...
def get_metrics_catalog():
    ...
    search_condition = (
        f"ucf.name LIKE :search OR ucf.formula LIKE :search" if search else ""
    )
    ...
    if is_aggregated:
        ...
        if "true" in agg_values or "1" in agg_values or "yes" in agg_values:
            agg_clauses.append("stats.instance_count > 1")
    ...
    if dedup_method_filter:
        ...
        if dedup_values:
            ...
            outer_conditions.append(
                "CASE "
                "WHEN stats.has_embedded = 1 AND stats.has_published = 1 THEN 'hash_mixed' "
                "WHEN stats.has_embedded = 1 THEN 'hash_embedded' "
                "ELSE 'hash_published' "
    ...
    base_sql = f"""
        SELECT 
            ucf.id as representative_id,
            ucf.name,
            ucf.formula,
            ucf.formula_hash,
            ucf.description,
            ucf.complexity_score as complexity,
            COALESCE(stats.instance_count, 1) as instance_count,
            COALESCE(stats.total_references, 0) as total_references,
            COALESCE(stats.total_usage, 0) as total_usage,
            stats.role,
            stats.data_type,
            COALESCE(stats.has_embedded, 0) as has_embedded,
            COALESCE(stats.has_published, 1) as has_published,
            ucf.id as unique_id
        ...
        LEFT JOIN (
            SELECT 
                ...
                MAX(CASE WHEN cf.datasource_id IN (SELECT id FROM datasources WHERE is_embedded = 1) THEN 1 ELSE 0 END) as has_embedded,
                MAX(CASE WHEN cf.datasource_id IN (SELECT id FROM datasources WHERE is_embedded = 0 OR is_embedded IS NULL) THEN 1 ELSE 0 END) as has_published
            FROM calculated_fields cf
            GROUP BY TRIM(cf.name), cf.formula_hash
        ) stats ON TRIM(ucf.name) = stats.clean_name AND ucf.formula_hash = stats.formula_hash
        WHERE 1=1 {"AND (" + search_condition + ")" if search else ""}
        {outer_where.replace("WHERE", "AND") if outer_where.startswith("WHERE") else outer_where}
    """

    # 统计总数 - 使用简化查询避免重复执行复杂 CTE
    count_sql = f"""
        SELECT COUNT(*) FROM (
    ...
    items = []
    for row in rows:
        has_embedded = bool(row.has_embedded)
        has_published = bool(row.has_published)
        ...
        dedup_key = (
            row.formula_hash[:8] + "..."
            if row.formula_hash and len(row.formula_hash) > 8
            else (row.formula_hash or "-")
        )
        ...
        items.append(
            {
                ...
                "usage_status": "direct"
                if (row.total_usage or 0) > 0
                else ("intermediate" if (row.total_references or 0) > 0 else "unused"),
                "datasources": [],
                "datasource_count": 0,
                "workbooks": [],
                "workbook_count": 0,
    ...
    facet_where = ""
    if search:
        facet_base_params["search"] = f"%{search}%"
        facet_where = "WHERE (cf.name LIKE :search OR cf.formula LIKE :search)"
    ...
    """
    role_counts = session.execute(text(role_sql), facet_base_params).fetchall()
    ...
    """
    try:
        dedup_counts = session.execute(
            text(dedup_method_sql), facet_base_params
        ).fetchall()
...
def get_metrics():
    ...
    if role_filter:
        role_values = [v.strip() for v in role_filter.split(",") if v.strip()]
        if len(role_values) == 1:
            conditions.append("cf.role = :role_filter")
            params["role_filter"] = role_values[0]
        elif len(role_values) > 1:
            role_placeholders = []
            for i, val in enumerate(role_values):
                key = f"role_{i}"
                params[key] = val
                role_placeholders.append(f":{key}")
            conditions.append(f"cf.role IN ({', '.join(role_placeholders)})")
    ...
    if sort == "complexity":
        order_clause = (
            f"ORDER BY cf.complexity_score {'DESC' if order == 'desc' else 'ASC'}"
        )
    elif sort == "referenceCount":
        order_clause = (
            f"ORDER BY cf.reference_count {'DESC' if order == 'desc' else 'ASC'}"
        )
    elif sort == "name":
...
def get_metric_detail(metric_id):
    ...
    if is_instance_mode:
        ...
        used_by_metrics = session.execute(
            text("""
            ...
            [
                {
                    ...
                    "is_certified": bool(metric_row.ds_certified)
                    if metric_row.ds_certified
                    else False,
                }
            ]
        ...
        data["all_workbooks"] = (
            [
                {
                    "id": metric_row.workbook_id,
                    "name": metric_row.workbook_name,
                    "project_name": metric_row.wb_project_name,
                    "owner": metric_row.wb_owner,
                }
            ]
            if metric_row.workbook_id
            else []
        )
        ...
        if metric_row.vizportal_url_id:
            data["tableau_url"] = build_tableau_url(
                "datasource", vizportal_url_id=metric_row.vizportal_url_id
            )
        else:
    ...
    if unique_row:
        ...
        data = {
            ...
            "dataType": metric_row.data_type,
            "complexity": metric_row.complexity_score or 0,
            "referenceCount": agg_stats.total_references
            if agg_stats
            else 0,  # 聚合统计
    ...
    if formula_hash:
        ...
        similar_result = session.execute(
            text("""
            ...
            LIMIT 50
        """),
            {"hash": formula_hash, "unique_id": unique_id},
        ).fetchall()
            ...
            similar.append(
                {
                    ...
                    "name": row[1],
                    "datasourceId": row[2],
                    "datasourceName": row[3] or "Unknown",
                    "usageCount": row[4] or 0,
                }
            )
    ...
    if unique_id:
        ...
        instances_result = session.execute(
            text("""
            SELECT 
        ...
        data["instances"] = [
            {
                ...
                "workbookId": row[3],
                "usageCount": row[4] or 0,
                "referenceCount": row[5] or 0,
                "dependencyCount": row[6] or 0,
                "datasourceName": row[7] or "-",
                "datasourceProject": row[8],
                "workbookName": row[9],
                "workbookProject": row[10],
            }
        ...
        # 3.1 聚合物理表血缘
        table_lineage_result = session.execute(
            text("""
        ...
        # 3.2 聚合数据源血缘
        ds_lineage_result = session.execute(
            text("""
        ...
        # 3.3 聚合工作簿血缘
        wb_lineage_result = session.execute(
            text("""
        ...
        used_by_metrics_result = session.execute(
            text("""
        ...
        impact_count_result = session.execute(
            text("""
        ...
        data["impact_metric_count"] = (
            impact_count_result.total if impact_count_result else 0
        )
        ...
        data["derived_tables"] = []
        data["derivedTables"] = []
        if "all_datasources" not in data:
            data["all_datasources"] = []
        if "all_workbooks" not in data:
            data["all_workbooks"] = []
        views_result = []
        data["used_by_metrics"] = []
    ...
    if vizportal_url_id:
        data["tableau_url"] = build_tableau_url(
            "datasource", vizportal_url_id=vizportal_url_id
        )
    else:
        data["tableau_url"] = None
...
def get_metric_impact_metrics(metric_id):
    ...
    rows = session.execute(
        ...
        LIMIT :limit OFFSET :offset
    """),
        {"unique_id": unique_id, "limit": page_size, "offset": offset},
    ).fetchall()
    ...
    items = [
        {
            ...
            "workbookId": row[7],
            "workbookName": row[8],
            "usageCount": row[9] or 0,
            "referenceCount": row[10] or 0,
        }
        for row in rows
    ]
