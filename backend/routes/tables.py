...
def get_tables():
    ...
    if sort == "name":
        query = query.order_by(
            DBTable.name.desc() if order == "desc" else DBTable.name.asc()
        )
    elif sort == "schema":
        query = query.order_by(
            DBTable.schema.desc() if order == "desc" else DBTable.schema.asc()
        )
    elif sort == "field_count":
        query = query.outerjoin(Field, DBTable.id == Field.table_id).group_by(
            DBTable.id
        )
        if order == "desc":
    ...
    if table_ids:
        ...
        field_stats = session.execute(
            field_stmt, {"table_ids": list(table_ids)}
        ).fetchall()
        field_stats_map = {
            row[0]: {
                "field_count": row[1],
                "measure_count": row[2],
                "dimension_count": row[3],
            }
    ...
    for t in tables:
        ...
        data["preview_fields"] = {
            ...
            "dimensions": [f"维度字段 ({dimension_count}个)"]
...
def get_table_detail(table_id):
    ...
    indirect_fields = (
        session.query(Field)
        .options(joinedload(Field.datasource))
    ...
    data["source_type"] = (
        "direct"
        if direct_fields
        else ("via_datasource" if table.datasources else "no_fields")
    )
