                ...
                upstream.append(
                    {
                        ...
                        "table_name": field.upstream_column.table.name
                        if field.upstream_column.table
                        else None,
                    }
                )
    ...
    return jsonify(
        {
            ...
            "labels": {
                "lineage_source": field.lineage_source
                if item_type == "field" and field
                else None,
                "penetration_status": field.penetration_status
                if item_type == "field" and field
                else None,
            },
        }
...
def global_search():
    ...
    unique_fields = (
        session.query(UniqueRegularField)
        .filter(
            (UniqueRegularField.name.ilike(search_pattern))
            | (UniqueRegularField.description.ilike(search_pattern))
        )
        .limit(limit)
        .all()
    )
    results["fields"] = [
        {
            ...
            "source": f.table.name if f.table else "-",
        }
    ...
    tables = (
        ...
        .filter(DBTable.name.ilike(search_pattern))
        .limit(limit)
        .all()
    )
    results["tables"] = [
    ...
    datasources = (
        ...
        .filter(Datasource.name.ilike(search_pattern))
        .limit(limit)
        .all()
    )
    results["datasources"] = [
    ...
    workbooks = (
        ...
        .filter(Workbook.name.ilike(search_pattern))
        .limit(limit)
        .all()
    )
    results["workbooks"] = [
    ...
    unique_metrics = (
        session.query(UniqueCalculatedField)
        .filter(
            (UniqueCalculatedField.name.ilike(search_pattern))
            | (UniqueCalculatedField.formula.ilike(search_pattern))
        )
        .limit(limit)
        .all()
    )
    results["metrics"] = [
        {
            ...
            "name": m.name,
            "type": "metric",
            "formula": m.formula[:50] + "..."
            if m.formula and len(m.formula) > 50
            else (m.formula or ""),
            "datasource": "-",
        }
    ...
    databases = (
        ...
        .filter(Database.name.ilike(search_pattern))
        .limit(limit)
        .all()
    )
    results["databases"] = [
        {
            ...
            "name": db.name,
            "type": "database",
            "connectionType": db.connection_type or "",
        }
        for db in databases
    ]
...
def get_lineage_graph(item_type, item_id):
    ...
    if item_type == "field":
        ...
        result = (
            session.query(Field, CalculatedField)
            .join(CalculatedField, Field.id == CalculatedField.id)
            .filter(Field.id == item_id)
            .first()
        )
        ...
        if result:
            ...
            if calc.formula and isinstance(calc.formula, str):
                ...
                for ref_name in list(set(refs))[:5]:
                    dep_field = (
                        session.query(Field).filter(Field.name == ref_name).first()
                    )
                    if dep_field:
...
def get_lineage_sankey():
    ...
    nodes = [
        ...
        # Layer 1: 数据表
        {
            "name": f"非嵌入式表 ({table_normal})",
            "depth": 1,
            "itemStyle": {"color": "#3b82f6"},
        },
        {
            "name": f"嵌入式表 ({table_embedded})",
            "depth": 1,
            "itemStyle": {"color": "#a855f7"},
        },
        {
            "name": f"孤立表 ({table_orphan})",
            "depth": 1,
            "itemStyle": {"color": "#f43f5e"},
        },
        # Layer 2: 已发布数据源
        {
            "name": f"正常发布数据源 ({ds_normal})",
            "depth": 2,
            "itemStyle": {"color": "#3b82f6"},
        },
        {
            "name": f"孤立发布数据源 ({ds_orphan})",
            "depth": 2,
            "itemStyle": {"color": "#f43f5e"},
        },
        {
            "name": f"CustomSQL数据源 ({ds_custom_sql})",
            "depth": 2,
            "itemStyle": {"color": "#f59e0b"},
        },
        # Layer 3: 嵌入式数据源
        {
            "name": f"嵌入式:引用已发布 ({ds_embed_ref})",
            "depth": 3,
            "itemStyle": {"color": "#a855f7"},
        },
        {
            "name": f"嵌入式:直连物理表 ({ds_embed_direct})",
            "depth": 3,
            "itemStyle": {"color": "#a855f7"},
        },
        # Layer 4: 原生字段
        {
            "name": f"原生字段 ({field_total:,})",
            "depth": 4,
            "itemStyle": {"color": "#3b82f6"},
        },
        # Layer 5: 计算字段
        {
            "name": f"计算字段 ({calc_total:,})",
            "depth": 5,
            "itemStyle": {"color": "#3b82f6"},
        },
        # Layer 6: 工作簿
        {
            "name": f"工作簿 ({workbook_count})",
            "depth": 6,
            "itemStyle": {"color": "#3b82f6"},
        },
        # Layer 7: 视图
        {
            "name": f"视图 ({view_count:,})",
            "depth": 7,
            "itemStyle": {"color": "#3b82f6"},
        },
    ]
    ...
    stats = {
        ...
        "calculated_fields": {"total": calc_total},
...
def merge_metrics():
    return jsonify({"error": "只读模式，治理操作已禁用"}), 405


@api_bp.route("/governance/deprecate", methods=["POST"])
def deprecate_metrics():
    return jsonify({"error": "只读模式，治理操作已禁用"}), 405
