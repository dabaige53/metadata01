...
**问题现象**：1,414 个原始字段的 `upstream_column_id` 在 `db_columns` 表中找不到对应记录。

**受影响的主要表**：

| 表名                            | 数据库       | 缺失列字段数 |
| ------------------------------- | ------------ | ------------ |
| dim_loc_agent_info              | data_sandbox | 727          |
| ods_mkt_oag_cr_report           | data_sandbox | 121          |
| DWD_ZY_Financial_Results_Detail | ho           | 86           |

**产生原因**：这些物理表的列没有被同步到 `db_columns` 表。可能是数据库视图，Tableau API 对视图的列采集支持有限。

**状态**：待修复。需要检查表同步逻辑，确保 GraphQL 查询包含 `columns` 字段。

#### 问题五：表引用字段无关联（待修复）

**问题现象**：约 900 个原始字段的 `table_id` 为空，但在 Tableau UI 中可以看到其上游表。

**典型示例**：

| 字段名   | 类型       | Tableau UI 显示              |
| -------- | ---------- | ---------------------------- |
| 日期表   | 表引用     | 能看到上游表 Dim-日期表.xlsx |
| 航司表   | 表引用     | 能看到上游表                 |
| DIM-进港 | 维度表引用 | 能看到上游表                 |

**产生原因**：Tableau Metadata API 对数据模型中的"表引用字段"返回的 `upstreamColumns` 为空。
