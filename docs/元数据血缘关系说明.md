...
**机制说明**：
在处理"穿透型"嵌入式数据源时，系统会自动建立双重关联，确保血缘完整：
1. 将上游发布源关联到工作簿（体现逻辑来源）
2. 将嵌入式数据源本身关联到工作簿（体现实体来源）

**技术实现**：
在保存嵌入式数据源后，系统显式调用 `_link_datasource_to_workbook`，确保 `datasource_to_workbook` 表中包含该记录，从而保证反向查询"表被哪些工作簿使用"时所有链路均正确连通。
...
**场景描述**：
部分嵌入式表或虚拟表的列信息可能不完整，导致 `upstream_column_id` 在 `db_columns` 表中找不到对应记录。

**处理策略**：
1. **穿透优先**：对于 `EmbeddedTable` 等类型，系统优先尝试穿透到上游物理表，使用物理表的有效列 ID。
2. **动态补全**：如果穿透后发现物理列记录不存在（如数据库视图未同步），系统在同步时会根据元数据自动创建缺失的 `DBTable` 和 `DBColumn` 记录。
3. **兜底保留**：对于无法穿透且无法补全的情况（如纯计算产生的中间表），保留原始引用并在前端做相应标记。

**验证指标**：
通过上述机制，系统确保所有字段均能建立有效的表级或列级关联，不存在 ID 断链情况。
...
**机制说明**：
在同步过程中，如果遇到字段引用的 `upstream_column_id` 在本地数据库中不存在（例如来自未被独立采集的数据库视图），系统会触发**自动补全机制**。

**处理逻辑**：
系统根据字段携带的元数据信息，自动在 `tables` 和 `db_columns` 表中创建缺失的记录，确保血缘链条不会因元数据采集的时间差或范围限制而中断。这一机制保证了字段始终能追溯到其直接上游。
...
**当前处理策略**：
1. **优先穿透**：尽可能找到背后的物理表。
2. **兜底策略**：如果无法穿透（如复杂自定义 SQL），则完整保留嵌入表信息，并将字段关联到该嵌入表。
3. **自动补全**：系统会自动补全缺失的嵌入表列信息，确保血缘链条完整。

**验证指标参考**：
- 修复前：大量字段因列信息缺失导致血缘中断（Warn 状态）
- 修复后：**血缘断链字段数为 0**。所有字段均成功建立到表的关联（物理表或嵌入表）。
...
**机制说明**：
为了准确处理不同类型的工作簿数据连接，系统采用**混合模式**（Hybrid Mode）来管理嵌入式数据源：

1. **穿透型**：引用了已发布数据源的副本。系统将其字段直接关联到上游发布源，避免数据冗余。
2. **独立直连型**：直接在工作簿中定义的数据连接（如直连 Excel）。系统将其视为独立实体完整保存，确保这类工作簿有正确的数据源和表关联。

**管理效果**：
该机制确保了所有工作簿——无论是连接服务器数据源还是本地文件——都能在系统中正确显示其"使用数据源"信息，杜绝了"孤立工作簿"现象。

#### 6.2 自定义 SQL 的血缘限制

**技术限制**：
对于使用 Custom SQL 的数据源，其血缘追溯能力取决于 Tableau Metadata API 的 SQL 解析能力。

**处理逻辑**：
- **可解析**：若 Tableau 能识别出 SQL 中的物理表，API 会返回 `upstreamTables`，系统将建立完整的"字段→表"血缘。
- **不可解析**：若 SQL 过于复杂，API 不返回上游表，系统将无法建立到物理表的关联。此时字段的 `table_id` 为空或指向 CustomSQL 节点，属于已知系统限制。
...
> [!IMPORTANT]
> 去重采用**双层缓存机制**：第一层按数据源+字段名，第二层按物理表名+列名。

**技术实现详解**：
在 `backend/tableau_sync.py` 的 `sync_fields` 方法中，维护了两个 Python 字典作为缓存：
1.  **`published_field_cache`** (一级缓存):
    *   **Key**: `(datasource_id, name)` —— 数据源 ID + 字段名
    *   **Value**: `field_id`
    *   **作用**: 识别穿透型嵌入式字段。如果一个嵌入式字段的 `(穿透后DS_ID, name)` 在此缓存中，说明它只是发布式字段的影子，直接跳过。

2.  **`physical_column_cache`** (二级缓存):
    *   **Key**: `(table_name, column_name)` —— 物理表名 + 列名
    *   **Value**: `field_id`
    *   **作用**: 识别独立直连型嵌入式字段。对于 Excel/CSV 等直连源，虽然数据源 ID 不同，但如果它们都指向同一个物理表（如同一个 CSV 文件名）的同一列，则通过此缓存进行物理层面的去重。
...
保存所有字段
建立缓存：
   - 填充 published_field_cache
   - 填充 physical_column_cache (解析 upstreamColumns 获取物理表信息)
   ↓
第二阶段：处理嵌入式数据源的字段
   ↓
检查一级缓存 (published_field_cache)
   ├─ 命中：跳过（这是穿透型副本）
   │
   └─ 未命中：检查二级缓存 (physical_column_cache)
         ├─ 命中：跳过（这是独立直连型副本，指向同一物理列）
         │
         └─ 未命中：保存（这是工作簿特有的新字段或计算字段）
```

**效果**：
该机制有效避免了因工作簿内嵌入副本导致的字段冗余，同时保证了独立直连型数据源及其字段的完整性，将系统中的字段数量控制在精确反映实际业务含义的水平。
...
**机制说明**：
为了解决嵌入式数据源（特别是独立直连型）可能被遗漏的问题，系统实施了严格的字段保留策略。只要判断为"独立直连型"数据源的字段，即使没有上游发布源，也会被完整保留并关联到其所属的嵌入式数据源。

#### 7.2 智能重连机制 (Smart Reconnection)

**机制说明**：
在去重过程中，部分嵌入式字段副本被过滤。为了确保视图（View）能正确关联到保留下来的有效字段，系统实现了智能重连：当视图引用了一个已被去重的字段 ID 时，系统会根据"工作簿+数据源+字段名"的上下文，自动将关联重定向到对应的已发布字段 ID。

#### 7.3 物理列自动同步

**机制说明**：
系统具备自动探测元数据缺失的能力。在处理字段血缘时，如果发现其依赖的 `upstream_column_id` 对应的物理列记录缺失（常见于数据库视图），系统会实时补全该列及其所属表的元数据记录。

**日志特征**：
- `🔨 补全缺失表: [表名] (Type: DatabaseTable)`
- `🔨 修复缺失物理列: [列名] -> [表名]`

#### 7.4 基于名称的表关联兜底

**机制说明**：
对于极少数 API 未返回 `upstreamColumns` 的字段（如特殊计算字段或表引用），系统启用基于名称的匹配兜底。如果字段名与系统已知的某个表名完全匹配，系统会自动建立该字段到表的关联，作为元数据缺失的最后一道保障。

**日志特征**：
- `🔨 修复无关联表字段: [字段名/表名] -> 关联到表 [表名]`
...
> [!NOTE]
> B1 和 D 类情况通过系统的自动补全和表名匹配机制处理，确保血缘完整。

| 类别              | 处理机制       | 说明                                           |
| ----------------- | -------------- | ---------------------------------------------- |
| A: 物理表+有效列  | 💡 标准流程     | 字段 → 物理列 → 物理表 → 数据库（完整链路）    |
| B1: 物理表+无效列 | 🔨 **自动补全** | 系统自动创建缺失的列和表记录（动态补全）       |
| C1: 嵌入表+有效列 | 💡 嵌入层追溯   | 字段 → 嵌入列 → 嵌入表（嵌入表本身为数据源头） |
| D: 无关联表       | 🔨 **名称匹配** | 通过表名匹配逻辑，自动关联引用字段到同名表     |
...
**策略**：
对于未引用发布源的嵌入式数据源（独立直连型），系统将其识别为顶层数据实体，完整保留其"数据源-表-工作簿"的三角关系，确保本地文件的直接连接也能被纳入治理范围，防止出现"孤立工作簿"。

### 4.2 字段归属的自动校正

**策略**：
系统根据数据源类型（穿透型 vs 独立型）自动判定字段归属：
- 穿透型：字段自动"归位"到上游发布源，防止产生冗余副本。
- 独立型：字段"锚定"在当前嵌入源，防止变成无主字段。

### 4.3 显式关联建立

**策略**：
在处理穿透型嵌入源时，系统不仅建立逻辑上的"上游发布源"关联，还强制建立物理上的"嵌入源本体"到工作簿的关联。这种双重链接确保了从任何维度（逻辑/物理）查询都能得到正确结果。

### 4.4 字段精简与去重

**策略**：
通过前述的"双层缓存机制"，系统在采集阶段即剔除冗余的影子字段，仅保留具有唯一业务含义的字段实例，大幅降低元数据存储压力。

### 4.5 视图关联的自愈能力

**策略**：
"智能重连"机制赋予了系统自愈能力。即使底层的字段 ID 发生变更（如因去重被合并），系统也能依靠上下文信息，将视图正确连接到有效的字段实体上，保证视图级分析的准确性。
...
| 挑战点               | 处理机制                 | 效果                                         |
| -------------------- | ------------------------ | -------------------------------------------- |
| 嵌入式数据源管理     | **混合模式 (Hybrid)**    | 准确区分发布源副本与独立直连源，无孤立与冗余 |
| 字段去重与关联       | **双层缓存 + 智能重连**  | 字段唯一化，且视图关联自动修正               |
| 元数据缺失 (API不全) | **自动补全 (Auto-fill)** | 自动创建缺失的物理列/表记录，修补血缘链      |
| 复杂表关联           | **名称匹配兜底**         | 对 API 无法返回引用的字段进行名称关联        |

## 二、已知技术限制

| 限制点              | 原因说明                   | 当前行为                                 |
| ------------------- | -------------------------- | ---------------------------------------- |
| Custom SQL 深度解析 | Tableau API 解析能力限制   | 若 API 无法识别表，则保留 CustomSQL 节点 |
| 复杂计算字段追溯    | 多层嵌套计算导致依赖树过深 | 尽可能回溯，部分复杂计算可能中断于中间层 |
