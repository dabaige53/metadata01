# 计算字段同步架构重构方案

> **版本**: v2.0  
> **日期**: 2025-12-24  
> **状态**: 待评审

---

# 第一部分：总体概述

## 一、背景与问题

### 1.1 当前架构分析

系统当前的 `backend/tableau_sync.py` 文件承担了过多职责，将数据采集、去重判断、血缘计算等逻辑耦合在一起。

**现有流程**：
```
调用 Tableau API 获取数据
    ↓
边采集边判断是否重复（在内存中维护缓存）
    ↓
├── 重复：跳过，不写入数据库
└── 不重复：写入数据库
    ↓
同步完成后，执行统计预计算
```

**问题分析**：

| 问题类型         | 具体表现                         | 产品影响                   |
| ---------------- | -------------------------------- | -------------------------- |
| **职责耦合**     | 采集代码中夹杂去重逻辑           | 难以调试、维护困难         |
| **数据丢失风险** | 重复项直接跳过，不保留原始记录   | 无法事后审计或调整去重策略 |
| **血缘断裂**     | 计算字段无物理表关联时直接跳过   | 用户看不到完整的数据来源   |
| **查询复杂**     | 展示时需要多表 JOIN 获取血缘信息 | API 响应慢、前端逻辑复杂   |

### 1.2 重构目标

1. **职责分离**：将采集、处理、预计算拆分为独立模块
2. **数据完整**：采集阶段全量存储，后置阶段标记去重
3. **血缘预存**：通过冗余列消除复杂 JOIN
4. **可追溯**：保留去重映射关系，支持事后审计

---

## 二、重构后架构

### 2.1 文件结构

```
backend/
├── tableau_collector.py   # [新建] 纯采集模块
├── metadata_processor.py  # [新建] 后置处理模块
├── tableau_sync.py        # [改造] 调度入口
└── models.py              # [修改] 新增冗余列
```

### 2.2 新流程设计

```
┌─────────────────────────────────────────────────────────────────┐
│  阶段1：纯采集（tableau_collector.py）                          │
├─────────────────────────────────────────────────────────────────┤
│  调用 Tableau API 获取数据                                       │
│      ↓                                                           │
│  全量写入数据库（不做任何去重判断）                              │
│  - 每个字段都保留原始 ID                                         │
│  - 即使是副本也完整存储                                          │
│  - 标记 is_raw = 1（原始采集状态）                               │
└─────────────────────────────────────────────────────────────────┘
                                ↓
┌─────────────────────────────────────────────────────────────────┐
│  阶段2：后置去重（metadata_processor.py）                        │
├─────────────────────────────────────────────────────────────────┤
│  扫描已采集的数据，识别重复项                                    │
│      ↓                                                           │
│  ├── 普通字段：按 (table_name, column_name) 去重                 │
│  └── 计算字段：按 (name, formula_hash) 去重                      │
│      ↓                                                           │
│  标记去重关系：                                                   │
│  - 设置 is_duplicate = 1（重复项）                               │
│  - 设置 survivor_id = 存活记录的 ID                              │
│  - 保留原始记录，不删除                                          │
└─────────────────────────────────────────────────────────────────┘
                                ↓
┌─────────────────────────────────────────────────────────────────┐
│  阶段3：血缘预计算（metadata_processor.py）                      │
├─────────────────────────────────────────────────────────────────┤
│  遍历所有字段，填充冗余血缘列                                    │
│      ↓                                                           │
│  ├── 普通字段：从 table_id → database_id 追溯                   │
│  └── 计算字段：递归解析公式，汇总所有依赖字段的物理表            │
│      ↓                                                           │
│  写入 lineage_* 冗余列                                           │
└─────────────────────────────────────────────────────────────────┘
                                ↓
┌─────────────────────────────────────────────────────────────────┐
│  阶段4：统计预存（metadata_processor.py）                        │
├─────────────────────────────────────────────────────────────────┤
│  更新 usage_count、metric_usage_count 等预计算字段               │
└─────────────────────────────────────────────────────────────────┘
```

---

# 第二部分：各模块详解

---

## 模块一：采集模块 (tableau_collector.py)

### 1. 基本职责

**业务含义**：纯粹的数据采集器，负责从 Tableau API 获取原始数据并全量写入数据库。

**设计原则**：
- 只负责"拉取 + 存储"
- 不做任何业务判断（去重、过滤等）
- 保留所有原始 ID

---

### 2. 类设计

```python
class TableauCollector:
    """Tableau 数据采集器（纯采集，无业务逻辑）"""
    
    def __init__(self, client: TableauMetadataClient, session):
        self.client = client
        self.session = session
    
    def collect_all(self):
        """执行全量采集"""
        self.collect_databases()
        self.collect_tables()
        self.collect_datasources()
        self.collect_workbooks()
        self.collect_views()
        self.collect_fields()
        self.collect_calculated_fields()
        self.collect_field_to_view_raw()
```

---

### 3. 采集方法详解

#### 3.1 collect_fields()

**职责**：采集所有字段（包括发布式和嵌入式），全量写入。

**与原有逻辑的区别**：

| 对比项   | 原有逻辑               | 新逻辑                           |
| -------- | ---------------------- | -------------------------------- |
| 去重判断 | 采集时判断，重复则跳过 | 不判断，全量写入                 |
| 缓存维护 | 内存中维护三级缓存     | 无缓存                           |
| 穿透处理 | 采集时穿透到发布源     | 仅记录原始信息，穿透交给后置处理 |

**详细过程**：
```
遍历所有数据源：
    ↓
调用 API 获取该数据源的所有字段
    ↓
对每个字段：
    ↓
    检查数据库中是否已存在该 ID
    ↓
    ├─ 存在：更新记录
    └─ 不存在：新增记录
    ↓
    保存以下字段：
       id = 字段原始 ID
       name = 字段名称
       datasource_id = 所属数据源 ID（原始值，不穿透）
       workbook_id = 所属工作簿 ID（如有）
       is_calculated = 是否计算字段
       formula = 公式（如有）
       ...其他属性
    ↓
    同时保存上游信息（用于后置处理）：
       raw_upstream_column_id = upstreamColumns[0].id（原始值）
       raw_upstream_table_id = upstreamColumns[0].table.id（原始值）
       raw_upstream_table_type = upstreamColumns[0].table.__typename
```

**代码示例**：
```python
def collect_fields(self) -> int:
    """采集所有字段（全量写入，不去重）"""
    print("\n📥 采集字段...")
    count = 0
    
    # 获取所有数据源（含嵌入式）
    datasources = self.session.query(Datasource).all()
    
    for ds in datasources:
        fields_data = self.client.fetch_fields_for_datasource(ds.id)
        
        for f_data in fields_data:
            if not f_data or not f_data.get("id"):
                continue
            
            # 获取或创建记录
            field = self.session.query(Field).filter_by(id=f_data["id"]).first()
            if not field:
                field = Field(id=f_data["id"])
                self.session.add(field)
            
            # 保存原始信息（不做穿透处理）
            field.name = f_data.get("name") or ""
            field.datasource_id = ds.id  # 原始数据源 ID
            field.is_calculated = f_data.get("isCalculated") or False
            field.formula = f_data.get("formula")
            
            # 保存上游原始信息（用于后置处理）
            upstream_cols = f_data.get("upstreamColumns") or []
            if upstream_cols:
                first_col = upstream_cols[0]
                field.raw_upstream_column_id = first_col.get("id")
                field.raw_upstream_column_name = first_col.get("name")
                if first_col.get("table"):
                    field.raw_upstream_table_id = first_col["table"].get("id")
                    field.raw_upstream_table_type = first_col["table"].get("__typename")
            
            count += 1
    
    self.session.commit()
    print(f"  ✅ 采集 {count} 个字段")
    return count
```

---

## 模块二：处理模块 (metadata_processor.py)

### 1. 基本职责

**业务含义**：对已采集的原始数据进行后置处理，包括去重标记、血缘预计算、统计预存。

**设计原则**：
- 只读取数据库中的已采集数据
- 不调用外部 API
- 所有操作可重复执行（幂等性）

---

### 2. 类设计

```python
class MetadataProcessor:
    """元数据后置处理器"""
    
    def __init__(self, session):
        self.session = session
        self.deduplication_map = {}  # skipped_id -> survivor_id
    
    def process_all(self):
        """执行全部处理"""
        self.deduplicate_fields()
        self.deduplicate_calculated_fields()
        self.compute_field_lineage()
        self.compute_stats()
```

---

### 3. 字段去重详解

#### 3.1 为什么要去重？

同一个字段在系统中可能存在多个副本：

| 场景                         | 说明                             | 副本数量                   |
| ---------------------------- | -------------------------------- | -------------------------- |
| 发布式数据源被多个工作簿引用 | 每个工作簿都会产生一个嵌入式副本 | 1 + N（1个发布 + N个嵌入） |
| 独立直连嵌入式数据源         | 多个工作簿各自连接同一个 Excel   | N（每个工作簿一个）        |
| 计算字段被复制               | 复制工作簿时，计算字段也被复制   | M（每次复制一个）          |

**如果不去重**：
- 搜索"销售额"会出现 N 个结果
- 无法确定哪个是"权威定义"
- 统计数据膨胀（字段数远超实际业务字段数）

#### 3.2 去重策略设计

> [!IMPORTANT]
> 去重采用**三级缓存机制**：第一级按数据源+字段名，第二级按物理表名+列名，第三级按公式哈希。

**三级缓存结构**：

| 级别 | 缓存名称                | Key 结构                    | Value      | 适用场景           |
| ---- | ----------------------- | --------------------------- | ---------- | ------------------ |
| 一级 | `published_field_cache` | `(datasource_id, name)`     | `field_id` | 穿透型嵌入式字段   |
| 二级 | `physical_column_cache` | `(table_name, column_name)` | `field_id` | 独立直连型普通字段 |
| 三级 | `calc_field_cache`      | `(name, formula_hash)`      | `field_id` | 计算字段           |

**去重流程**：

```
扫描 fields 表中所有记录：
    ↓
第一阶段：识别发布式数据源字段（构建基准）
    ↓
    遍历所有 is_embedded = 0 的数据源的字段：
    ↓
    对每个字段：
        ├── 加入一级缓存：published_field_cache[(ds_id, name)] = field_id
        ├── 解析 raw_upstream_column 信息
        │   └── 加入二级缓存：physical_column_cache[(table_name, col_name)] = field_id
        └── 如果是计算字段：
            计算 formula_hash = MD5(normalize(formula))
            加入三级缓存：calc_field_cache[(name, formula_hash)] = field_id
    ↓
第二阶段：扫描嵌入式数据源字段（查重标记）
    ↓
    遍历所有 is_embedded = 1 的数据源的字段：
    ↓
    对每个字段：
        ↓
        检查一级缓存（穿透型副本）：
            获取该嵌入源的 source_published_datasource_id
            key = (source_published_datasource_id, name)
            ↓
            ├── 命中：标记 is_duplicate = 1, survivor_id = 缓存值
            │         记录 deduplication_map[当前ID] = 缓存值
            │         continue（跳过后续检查）
            │
            └── 未命中：继续检查二级缓存
        ↓
        检查二级缓存（独立直连型普通字段）：
            从 raw_upstream_table_name + raw_upstream_column_name 构建 key
            ↓
            ├── 命中：标记 is_duplicate = 1, survivor_id = 缓存值
            │
            └── 未命中：继续检查三级缓存（如果是计算字段）
        ↓
        检查三级缓存（计算字段）：
            计算 formula_hash
            key = (name, formula_hash)
            ↓
            ├── 命中：标记 is_duplicate = 1, survivor_id = 缓存值
            │
            └── 未命中：这是新字段，保留
                加入对应缓存
```

**代码示例**：
```python
def deduplicate_fields(self):
    """字段去重（后置标记，不删除）"""
    print("\n⚙️ 执行字段去重...")
    
    # 初始化缓存
    published_field_cache = {}   # (ds_id, name) -> field_id
    physical_column_cache = {}   # (table_name, col_name) -> field_id
    calc_field_cache = {}        # (name, formula_hash) -> field_id
    
    # 第一阶段：处理发布式数据源字段，构建缓存
    published_ds_ids = self.session.execute(
        text("SELECT id FROM datasources WHERE is_embedded = 0")
    ).scalars().all()
    
    published_fields = self.session.query(Field).filter(
        Field.datasource_id.in_(published_ds_ids)
    ).all()
    
    for f in published_fields:
        # 一级缓存
        if f.datasource_id and f.name:
            published_field_cache[(f.datasource_id, f.name)] = f.id
        
        # 二级缓存
        if f.raw_upstream_table_name and f.raw_upstream_column_name:
            key = (f.raw_upstream_table_name, f.raw_upstream_column_name)
            physical_column_cache[key] = f.id
        
        # 三级缓存（计算字段）
        if f.is_calculated and f.formula:
            norm_formula = "".join(f.formula.split()).lower()
            formula_hash = hashlib.md5(norm_formula.encode()).hexdigest()
            calc_field_cache[(f.name, formula_hash)] = f.id
    
    # 第二阶段：处理嵌入式数据源字段，查重标记
    embedded_ds = self.session.query(Datasource).filter_by(is_embedded=True).all()
    
    duplicate_count = 0
    for ds in embedded_ds:
        fields = self.session.query(Field).filter_by(datasource_id=ds.id).all()
        
        for f in fields:
            survivor_id = None
            
            # 检查一级缓存
            if ds.source_published_datasource_id:
                key = (ds.source_published_datasource_id, f.name)
                if key in published_field_cache:
                    survivor_id = published_field_cache[key]
            
            # 检查二级缓存
            if not survivor_id:
                if f.raw_upstream_table_name and f.raw_upstream_column_name:
                    key = (f.raw_upstream_table_name, f.raw_upstream_column_name)
                    if key in physical_column_cache:
                        survivor_id = physical_column_cache[key]
            
            # 检查三级缓存
            if not survivor_id and f.is_calculated and f.formula:
                norm_formula = "".join(f.formula.split()).lower()
                formula_hash = hashlib.md5(norm_formula.encode()).hexdigest()
                key = (f.name, formula_hash)
                if key in calc_field_cache:
                    survivor_id = calc_field_cache[key]
            
            # 标记重复
            if survivor_id and survivor_id != f.id:
                f.is_duplicate = True
                f.survivor_id = survivor_id
                self.deduplication_map[f.id] = survivor_id
                duplicate_count += 1
            else:
                # 新字段，加入缓存
                if f.raw_upstream_table_name and f.raw_upstream_column_name:
                    key = (f.raw_upstream_table_name, f.raw_upstream_column_name)
                    if key not in physical_column_cache:
                        physical_column_cache[key] = f.id
                
                if f.is_calculated and f.formula:
                    norm_formula = "".join(f.formula.split()).lower()
                    formula_hash = hashlib.md5(norm_formula.encode()).hexdigest()
                    key = (f.name, formula_hash)
                    if key not in calc_field_cache:
                        calc_field_cache[key] = f.id
    
    self.session.commit()
    print(f"  ✅ 标记 {duplicate_count} 个重复字段")
```

---

### 4. 血缘预计算详解

#### 4.1 为什么要预计算血缘？

**问题**：字段的血缘信息分散在多个表中，查询时需要多层 JOIN。

**现有查询复杂度**：
```sql
-- 获取字段的完整血缘信息
SELECT f.*, 
       col.name AS column_name,
       t.name AS table_name, t.schema AS table_schema,
       db.name AS database_name,
       ds.name AS datasource_name
FROM fields f
LEFT JOIN db_columns col ON f.upstream_column_id = col.id
LEFT JOIN tables t ON f.table_id = t.id
LEFT JOIN databases db ON t.database_id = db.id
LEFT JOIN datasources ds ON f.datasource_id = ds.id
WHERE f.id = ?
```

**预计算后**：
```sql
-- 直接读取冗余列
SELECT * FROM fields WHERE id = ?
-- 所有血缘信息已在 lineage_* 列中
```

#### 4.2 血缘预计算流程

**普通字段处理**：
```
遍历所有非计算字段：
    ↓
从 raw_upstream_table_id 获取表信息：
    ↓
    检查表类型 (raw_upstream_table_type)：
    ↓
    ├── DatabaseTable（物理表）：
    │       直接使用该表信息
    │       填充 lineage_table_id = 表 ID
    │       填充 lineage_table_name = 表名
    │       ↓
    │       从表的 database_id 获取数据库信息
    │       填充 lineage_database_id = 库 ID
    │       填充 lineage_database_name = 库名
    │
    └── EmbeddedTable / CustomSQLTable（嵌入式表）：
            尝试穿透：
            ↓
            查询该表的 upstreamTables
            ↓
            ├── 有值：取第一个物理表，填充血缘信息
            └── 无值：填充嵌入表信息，标记 lineage_path = "Embedded (无法穿透)"
```

**计算字段处理（递归追溯）**：
```
遍历所有计算字段：
    ↓
解析公式中的依赖字段：
    使用正则 \[(.*?)\] 提取所有引用
    例如 "[销售额] / [订单数]" → ["销售额", "订单数"]
    ↓
递归查找依赖字段的物理表：
    ↓
    对每个依赖字段：
    ├── 如果是普通字段：获取其 lineage_table_id
    └── 如果是计算字段：递归解析其依赖
    ↓
汇总所有底层物理表：
    去重后得到 table_id 列表
    ↓
填充血缘信息：
    lineage_table_id = 第一个表 ID（主依赖）
    lineage_all_table_ids = JSON 数组（所有依赖表）
    lineage_path = "CalcField → 销售额 → Table A; 订单数 → Table B"
```

**代码示例**：
```python
def compute_field_lineage(self):
    """预计算字段血缘（填充冗余列）"""
    print("\n⚙️ 预计算字段血缘...")
    
    # 构建辅助查找表
    tables = {t.id: t for t in self.session.query(DBTable).all()}
    databases = {db.id: db for db in self.session.query(Database).all()}
    datasources = {ds.id: ds for ds in self.session.query(Datasource).all()}
    
    fields = self.session.query(Field).filter_by(is_duplicate=False).all()
    
    for f in fields:
        # 填充数据源名称
        if f.datasource_id and f.datasource_id in datasources:
            f.lineage_datasource_name = datasources[f.datasource_id].name
        
        if not f.is_calculated:
            # 普通字段：直接追溯
            self._compute_direct_lineage(f, tables, databases)
        else:
            # 计算字段：递归追溯
            self._compute_calc_lineage(f, tables, databases)
    
    self.session.commit()
    print(f"  ✅ 预计算 {len(fields)} 个字段的血缘")

def _compute_direct_lineage(self, field, tables, databases):
    """计算普通字段的直接血缘"""
    table_id = field.raw_upstream_table_id
    table_type = field.raw_upstream_table_type
    
    # 嵌入式表穿透
    if table_type in ("EmbeddedTable", "CustomSQLTable"):
        if table_id and table_id in tables:
            tbl = tables[table_id]
            # 尝试从 upstreamTables 获取物理表
            # ... (穿透逻辑)
    
    # 填充表信息
    if table_id and table_id in tables:
        tbl = tables[table_id]
        field.lineage_table_id = tbl.id
        field.lineage_table_name = tbl.name
        field.lineage_table_schema = tbl.schema
        
        # 填充数据库信息
        if tbl.database_id and tbl.database_id in databases:
            db = databases[tbl.database_id]
            field.lineage_database_id = db.id
            field.lineage_database_name = db.name
        
        field.lineage_path = f"Field → {tbl.name} → {db.name if db else '?'}"

def _compute_calc_lineage(self, field, tables, databases, visited=None):
    """计算计算字段的递归血缘"""
    if visited is None:
        visited = set()
    
    if field.id in visited:
        return set()  # 避免循环依赖
    visited.add(field.id)
    
    # 解析公式中的依赖
    formula = field.formula or ""
    refs = re.findall(r'\[(.*?)\]', formula)
    
    all_table_ids = set()
    
    for ref_name in set(refs):
        # 查找依赖字段
        dep_field = self.session.query(Field).filter_by(
            datasource_id=field.datasource_id,
            name=ref_name,
            is_duplicate=False
        ).first()
        
        if dep_field:
            if dep_field.is_calculated:
                # 递归
                sub_tables = self._compute_calc_lineage(dep_field, tables, databases, visited)
                all_table_ids.update(sub_tables)
            else:
                # 普通字段，获取其表
                if dep_field.lineage_table_id:
                    all_table_ids.add(dep_field.lineage_table_id)
    
    # 填充血缘信息
    if all_table_ids:
        first_table_id = list(all_table_ids)[0]
        field.lineage_table_id = first_table_id
        if first_table_id in tables:
            tbl = tables[first_table_id]
            field.lineage_table_name = tbl.name
            if tbl.database_id and tbl.database_id in databases:
                field.lineage_database_name = databases[tbl.database_id].name
        
        field.lineage_all_table_ids = json.dumps(list(all_table_ids))
        field.lineage_path = f"CalcField → {len(all_table_ids)} tables"
    
    return all_table_ids
```

---

# 第三部分：数据库设计变更

## 一、fields 表新增列

### 1.1 采集阶段原始信息列

| 列名                       | 类型         | 说明           | 填充时机 |
| -------------------------- | ------------ | -------------- | -------- |
| `raw_upstream_column_id`   | VARCHAR(255) | 上游列原始 ID  | 采集阶段 |
| `raw_upstream_column_name` | VARCHAR(255) | 上游列原始名称 | 采集阶段 |
| `raw_upstream_table_id`    | VARCHAR(255) | 上游表原始 ID  | 采集阶段 |
| `raw_upstream_table_type`  | VARCHAR(50)  | 上游表类型     | 采集阶段 |

### 1.2 去重标记列

| 列名           | 类型         | 说明                | 填充时机 |
| -------------- | ------------ | ------------------- | -------- |
| `is_duplicate` | BOOLEAN      | 是否为重复项        | 去重阶段 |
| `survivor_id`  | VARCHAR(255) | 重复项指向的存活 ID | 去重阶段 |

### 1.3 血缘冗余列

| 列名                      | 类型         | 说明                    | 填充时机   |
| ------------------------- | ------------ | ----------------------- | ---------- |
| `lineage_column_name`     | VARCHAR(255) | 上游物理列名            | 血缘预计算 |
| `lineage_table_id`        | VARCHAR(255) | 上游物理表 ID（穿透后） | 血缘预计算 |
| `lineage_table_name`      | VARCHAR(255) | 上游物理表名            | 血缘预计算 |
| `lineage_table_schema`    | VARCHAR(255) | 上游物理表 Schema       | 血缘预计算 |
| `lineage_database_id`     | VARCHAR(255) | 上游数据库 ID           | 血缘预计算 |
| `lineage_database_name`   | VARCHAR(255) | 上游数据库名            | 血缘预计算 |
| `lineage_datasource_name` | VARCHAR(255) | 所属数据源名            | 血缘预计算 |
| `lineage_workbook_name`   | VARCHAR(255) | 所属工作簿名            | 血缘预计算 |
| `lineage_all_table_ids`   | TEXT         | 所有依赖表 ID（JSON）   | 血缘预计算 |
| `lineage_path`            | TEXT         | 完整血缘路径描述        | 血缘预计算 |

---

## 二、DDL 变更脚本

```sql
-- ========== 阶段1：采集阶段原始信息列 ==========

ALTER TABLE fields ADD COLUMN raw_upstream_column_id VARCHAR(255);
ALTER TABLE fields ADD COLUMN raw_upstream_column_name VARCHAR(255);
ALTER TABLE fields ADD COLUMN raw_upstream_table_id VARCHAR(255);
ALTER TABLE fields ADD COLUMN raw_upstream_table_type VARCHAR(50);

-- ========== 阶段2：去重标记列 ==========

ALTER TABLE fields ADD COLUMN is_duplicate BOOLEAN DEFAULT 0;
ALTER TABLE fields ADD COLUMN survivor_id VARCHAR(255);

-- ========== 阶段3：血缘冗余列 ==========

ALTER TABLE fields ADD COLUMN lineage_column_name VARCHAR(255);
ALTER TABLE fields ADD COLUMN lineage_table_id VARCHAR(255);
ALTER TABLE fields ADD COLUMN lineage_table_name VARCHAR(255);
ALTER TABLE fields ADD COLUMN lineage_table_schema VARCHAR(255);
ALTER TABLE fields ADD COLUMN lineage_database_id VARCHAR(255);
ALTER TABLE fields ADD COLUMN lineage_database_name VARCHAR(255);
ALTER TABLE fields ADD COLUMN lineage_datasource_name VARCHAR(255);
ALTER TABLE fields ADD COLUMN lineage_workbook_name VARCHAR(255);
ALTER TABLE fields ADD COLUMN lineage_all_table_ids TEXT;
ALTER TABLE fields ADD COLUMN lineage_path TEXT;

-- ========== 索引优化 ==========

CREATE INDEX idx_fields_is_duplicate ON fields(is_duplicate);
CREATE INDEX idx_fields_survivor ON fields(survivor_id);
CREATE INDEX idx_fields_lineage_table ON fields(lineage_table_id);
CREATE INDEX idx_fields_lineage_database ON fields(lineage_database_id);
```

---

# 第四部分：实施步骤

## 一、阶段划分

### 阶段1：代码重构（预计 2 天）

| 步骤 | 操作                              | 验证点                    |
| ---- | --------------------------------- | ------------------------- |
| 1.1  | 创建 `tableau_collector.py`       | 文件存在，类结构完整      |
| 1.2  | 从 `tableau_sync.py` 提取采集逻辑 | 采集方法正确复制          |
| 1.3  | 移除采集代码中的去重逻辑          | 无 `cache`、`skip` 关键字 |
| 1.4  | 创建 `metadata_processor.py`      | 文件存在，类结构完整      |
| 1.5  | 实现 `deduplicate_fields()`       | 三级缓存逻辑完整          |
| 1.6  | 实现 `compute_field_lineage()`    | 递归追溯逻辑完整          |

### 阶段2：数据库变更（预计 0.5 天）

| 步骤 | 操作                  | 验证点                 |
| ---- | --------------------- | ---------------------- |
| 2.1  | 备份现有数据库        | `metadata.db.bak` 存在 |
| 2.2  | 更新 `models.py`      | 新增列定义完整         |
| 2.3  | 执行 DDL 脚本         | 列存在，索引创建成功   |
| 2.4  | 更新 `to_dict()` 方法 | API 返回新字段         |

### 阶段3：调度改造（预计 0.5 天）

| 步骤 | 操作                   | 验证点           |
| ---- | ---------------------- | ---------------- |
| 3.1  | 改造 `tableau_sync.py` | 调用新模块       |
| 3.2  | 更新 `sync_all()` 方法 | 四阶段执行       |
| 3.3  | 添加日志输出           | 各阶段有明确标识 |

### 阶段4：API 简化（预计 1 天）

| 步骤 | 操作                        | 验证点                  |
| ---- | --------------------------- | ----------------------- |
| 4.1  | 简化 `get_field_detail()`   | 直接读取冗余列          |
| 4.2  | 简化 `get_fields()`         | 过滤 `is_duplicate = 0` |
| 4.3  | 简化 `metrics/catalog`      | 使用预计算数据          |
| 4.4  | 简化前端 `DetailDrawer.tsx` | 移除复杂 JOIN 逻辑      |

---

## 二、验证计划

### 2.1 单元测试

```bash
# 测试采集完整性
python3 -c "
from backend.tableau_collector import TableauCollector
# 执行采集
# 验证 fields 表包含所有原始记录
"

# 测试去重正确性
python3 -c "
from backend.metadata_processor import MetadataProcessor
# 执行去重
# 验证 is_duplicate 正确标记
"
```

### 2.2 集成测试

```bash
# 执行全量同步
python3 run_backend.py --task sync

# 检查采集结果（应包含所有记录，含副本）
sqlite3 backend/metadata.db "
SELECT COUNT(*) as total,
       SUM(CASE WHEN is_duplicate = 1 THEN 1 ELSE 0 END) as duplicates
FROM fields
"

# 检查血缘填充
sqlite3 backend/metadata.db "
SELECT COUNT(*) as has_lineage
FROM fields 
WHERE is_duplicate = 0 AND lineage_table_name IS NOT NULL
"
```

### 2.3 回归测试

| 测试项           | 预期结果                  |
| ---------------- | ------------------------- |
| 搜索"高峰类型"   | 只显示 1 个（非重复）结果 |
| 查看计算字段详情 | "所属数据表"正确显示      |
| 查看普通字段详情 | 血缘完整，含数据库名      |

---

## 三、风险与回滚

### 3.1 风险点

| 风险             | 影响           | 缓解措施               |
| ---------------- | -------------- | ---------------------- |
| 数据库变更不兼容 | 旧版本无法运行 | 添加列而非删列         |
| 去重逻辑错误     | 误标记有效字段 | 只标记不删除，支持重置 |
| 同步时间增加     | 用户等待变长   | 分阶段显示进度         |

### 3.2 回滚方案

```bash
# 恢复数据库
mv backend/metadata.db.bak backend/metadata.db

# 恢复代码
git checkout HEAD~1 -- backend/tableau_sync.py

# 重置去重标记
sqlite3 backend/metadata.db "
UPDATE fields SET is_duplicate = 0, survivor_id = NULL
"
```

---

# 附录

## A. 相关文件清单

| 文件                                       | 变更类型 | 说明         |
| ------------------------------------------ | -------- | ------------ |
| `backend/tableau_collector.py`             | 新建     | 纯采集模块   |
| `backend/metadata_processor.py`            | 新建     | 后置处理模块 |
| `backend/tableau_sync.py`                  | 改造     | 调度入口     |
| `backend/models.py`                        | 修改     | 新增列定义   |
| `backend/routes/api.py`                    | 简化     | 使用冗余列   |
| `frontend/src/components/DetailDrawer.tsx` | 简化     | 移除复杂逻辑 |

## B. 参考文档

- [元数据血缘关系说明](../元数据血缘关系说明.md)
- [字段采集去重关联流程](../字段采集去重关联流程.md)
