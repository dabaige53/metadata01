...
4. 物理表→发布源：反向 `tables.is_embedded=0` + publish ds；`cross >= main`  
5. 物理表→嵌入源(独立)：反向 `tables.is_embedded=0` + embed independent ds；`cross >= main`  
6. 物理表→字段：`COUNT(DISTINCT t.id WHERE EXISTS fields f.table_id=t.id)`；`cross >= main`
...
### 4.11 计算字段（6项）
46. 计算字段→物理表：主口径 vs LFL(`table_id IN phys`)，附 `multi_table_cnt` 不下降  
...
50. 计算字段→工作簿：主口径 vs LFL(`workbook_id`)；`cross >= main`  
